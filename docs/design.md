...
- 自動再送: `tries=3`、`backoff=[60, 300, 900]`（1分・5分・15分）
- 3回失敗したら `mail_logs.status=failed`、`last_error` に最終エラーを保存
- 管理画面 `MailLogs` に「再送」アクションを置き、手動で再投入できるようにする
- 手動再送は 1 操作につき 1 回だけジョブを積み、実際の再試行回数はジョブ設定に任せる
...
`AdjustShopifyInventory` と `SendReservationMail` が最終失敗したジョブは、管理画面の
失敗件数ウィジェットに表示し、当日中に手動再実行する運用にする。
