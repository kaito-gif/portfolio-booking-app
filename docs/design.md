...
何度流しても同じ結果になるようにする（冪等）。このシステムは本番URL自体が
公開デモ環境であり、`APP_ENV=production` の cron から毎日3:00に実行される想定のため、
`production` 環境で実行を拒否するガードは入れない（2026-08-07、このガードが原因で
デモアカウントが一度も作成されずログイン不能になる障害が発生し、削除した）。
